...
//Выполните то, что есть в этой лабораторной. Попробуйте зарегистрировать и запустить сервис. Запуск выполните из окна служб. ВНИМАНИЕ: запрограммируйте сервис так, чтобы при его останове выполнялось завершение работы системы
...
int main()
{
	SERVICE_TABLE_ENTRY ServiceTable[1]; //описывает точку входа для сервис - менеджера
	ServiceTable[0].lpServiceName = (LPWSTR)TEXT("SERV_G");
	ServiceTable[0].lpServiceProc = (LPSERVICE_MAIN_FUNCTION)ServiceStart;

	if (!StartServiceCtrlDispatcher(ServiceTable)) //связывает наш сервис с SCM (Service Control Manager). 
	{
...
void WINAPI ServiceStart(DWORD argc, LPTSTR* argv)
{
	...
	serviceStatusHandle = RegisterServiceCtrlHandler(TEXT("SERV_G"), ServiceCtrlHandler); //регистрация сервиса
	...
	if (!SetServiceStatus(serviceStatusHandle, &serviceStatus)) //. Для изменения статуса сервиса 
	{
		status = GetLastError();
...
int addLogMessage(const char* text)
{
	...
	out.open("D:\\Study\\OSaSP-2\\lab13\\Debug"); // окрываем файл для записи
	if (out.is_open())
	{
		out << text << std::endl;
	}
